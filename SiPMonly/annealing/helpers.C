...
void readtemperature_directly(TString file,
                     double &center, double &error) {
  std::cout << "File: " << file.Data() << std::endl;
  vector<double> resist;
  std::ifstream fin( Form("%s",file.Data()) );
  std::string line;
  for(;;) {
    if(!std::getline(fin,line)) break;
    TString rline = line;
    TObjArray *lst = rline.Tokenize(",");
    if(lst->GetEntries()!=5) continue;
    resist.push_back( ((TObjString*) lst->At(1))->GetString().Atof() );
  }
  std::cout << "I read " << resist.size() << " values of temperature " << std::endl;
  sort( resist.begin(), resist.end() );
  double temp1 = resist[3];
  double temp2 = resist[resist.size()-4];
  std::cout << "First value " << temp1 << " last value " << temp2 << std::endl;
  center = 0.5*(temp1+temp2);
  error = 0.5*(temp2-temp1);
}
...
void subtractreference2( int n, double c[MAX],
			 double cref[MAX],
			 double out[MAX] ) {
  for( int i=0; i!=n; ++i )
    out[i] = c[i] - cref[i];
